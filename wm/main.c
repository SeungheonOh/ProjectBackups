...
void btn_release(XEvent *e){
	...
	XMoveResizeWindow(d, current.window, 
			MIN(mouse.x_root, e->xbutton.x_root), 
			MIN(mouse.y_root, e->xbutton.y_root), 
...
void map_request(XEvent *e){
	int x = 0, y = 0;
	if(!XQueryPointer(d, root, &(Window){0}, &(Window){0}, &x, &y, &(int){0}, &(int){0}, &(int){0})) return;
...
void motion_notify(XEvent *e) {
	...
	if(!current.window || !mouse.subwindow){
		...
		outline(MIN(mouse.x_root, e->xbutton.x_root), 
			MIN(mouse.y_root, e->xbutton.y_root), 
			ABS(xd), 
			ABS(yd));

 ...
 * all directions, thus making it look like it is inflating.
...
r.width = W;
r.height = H;
xcb_poly_rectangle(conn, wid, gc,, &r);
 ...
 * The special mouse buttons/5 (scroll up/down) are treated especially,
 ...
 */
 ...
 {
int mask;
static xcb_timestamp_t lasttime =;
...
if (xcb_cursor_context_new(conn, scrn, &cx) <) {
 ...
 * Events are reported for every single move by pixel.
 *
...
if (verbose)
fprintf(stderr, "%sx%08xx%08x:%dx%d+%d+%d\n",
...
if (verbose)
fprintf(stderr, "%sx%08x %dx%d+%d+%d\n",
XEV(e), e->window,
e->width, e->height,
e->x, e->y);
...
xcb_grab_button(conn,, scrn->root, XCB_EVENT_MASK_BUTTON_PRESS,
